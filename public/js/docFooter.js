    ...
    render:function(){
        return (
            <div className="yourComments">
                ...
                <form action={this.state.isSigned ? "/api/evaPost" : "/api/registerPost"} method="post" encType="multipart/form-data" role="form">
                    {
                        (() => this.state.isSigned?
                            <div>
                                <textarea name="comments" className="doc_comments" cols="30" rows="10" placeholder="请输入您的评论"></textarea>
                            </div>:
                            <div>
                                <input name="visitorName" className="form-control" type="text" placeholder="请输入您的姓名（必填）" required="required"/>
                                <input name="visitorAddress" className="form-control"  type="text" placeholder="请输入您的邮箱地址（必填）" required="required"/>
                                <input name="visitorWeb" className="form-control"  type="text" placeholder="请输入您的网址"/>
                            </div>
                        )()
                    }
...
var Like = React.createClass({
    ...
    render:function () {
        return (
            <div className={this.state.isActiveLike || (this.state.isFirst && this.props.likeActive == 'true')?'glyphicon glyphicon-thumbs-up like active':'glyphicon glyphicon-thumbs-up like'} onClick={this.changeLike}><span>{this.state.num}</span></div>
        )
    }
});
var Comments = React.createClass({
    ...
    getInitialState:() => {return {comments:[],likeArr:[]}},
    componentDidMount:function(){
        this.getCommentsMsg();
    },
    render:function(){
                        ...
                        commentsMsg.map((obj,index) =>
                            <li key={index}>
                                <div>
                                    ...
                                    <Like like={obj.like} index={index} likeActive={user && this.state.likeArr?this.state.likeArr[index]:''} />
                                </div>
